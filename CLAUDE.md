...
Tauri desktop app: pixel art office where AI agents (Claude Code terminals) are animated characters.
...
src/                          — Extension backend (Node.js, VS Code API)
  ...
  monitor/                    — Session monitor pipeline (OpenCode/Codex ingestion, normalization, snapshotting)
    monitorController.ts      — Poll loop + notifications
    agentStore.ts             — Aggregated agent/session state + snapshot view
    sources/                  — opencode/codex file discovery and event extraction
...
**Monitor throughput guard**: Monitor source discovery is capped to the latest 20 files/sessions (`MONITOR_SOURCE_FILE_LIMIT`), and snapshots retain only the latest 20 agents to keep CPU usage stable on large history directories.
...
**Sub-agents**: Negative IDs (from -1 down). Created on `agentToolStart` with "Subtask:" prefix. Same palette + hueShift as parent. Click focuses parent terminal. Not persisted. Spawn prefers a discussion ring around the parent (walkable, unoccupied nearby offsets) and faces toward the parent to look like a group discussion; fallback: closest walkable tile. **Sub-agent permission detection**: when a sub-agent runs a non-exempt tool, `startPermissionTimer` fires on the parent agent; if 5s elapse with no data, permission bubbles appear on both parent and sub-agent characters. `activeSubagentToolNames` (parentToolId → subToolId → toolName) tracks which sub-tools are active for the exempt check. Cleared when data resumes or Task completes.
